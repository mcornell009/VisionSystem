...
// CALIBRATE THE STEREO CAMERAS
    ...
    FileStorage fs("intrinsics_test.yml", CV_STORAGE_WRITE);
    if( fs.isOpened() )
    {
        fs << "M1" << &_M1 << "D1" << &_D1 <<
            "M2" << &_M2 << "D2" << &_D2;
...
/*
            cvStereoRectify(&_M1, &_D1, &_M2, &_D2,
                imageSize, &_R, &_T, &_R1, &_R2, &_P1, &_P2, &_Q,
                1);
                //, -1,imageSize, &validRoi[0], &validRoi[1]);
*/
            
            fs.open("extrinsics_test.yml", CV_STORAGE_WRITE);
            if( fs.isOpened() )
            {
...
int main(void)
{
    StereoCalib("calib_list.txt", 9, 7, 0);
    return 0;
}
